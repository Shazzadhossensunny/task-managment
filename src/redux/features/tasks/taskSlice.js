...
export const addTask = createAsyncThunk(
  ...
  async (task,  { dispatch }) => {
    const newTask = {
      ...task,
      completed: false,  // Set the default status to pending
    };
...
export const updateTask = createAsyncThunk(
  'tasks/updateTask',
  async ({ id, updates }) => {
    console.log('Updating Task:', { id, updates }); // Debugging logs
  ...
  reducers: {
    setFilter: (state, action) => {
      ...
      state.filteredItems = state.items.filter(task => {
        ...
        const matchesSearch = taskTitle.includes(state.filters.search.toLowerCase()) ||
          taskDescription.includes(state.filters.search.toLowerCase());
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchTasks.fulfilled, (state, action) => {
        ...
        state.filteredItems = action.payload; // Default filtered items to all tasks
      })
      ...
      .addCase(addTask.fulfilled, (state, action) => {
        ...
        state.filteredItems.push(action.payload); // Add new task to filtered list
      })
